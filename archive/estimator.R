...
estimate.quantile <- function(Y, Xt, Z = NULL, alpha = 0.5, x = NULL, method = "BFGS", ...) {
    ntime <- nrow(Y)
    nloc <- ncol(Y)
    Y.mean <- colMeans(Y)
    if (is.null(dim(Xt))) {
    ...
    Y.demean <- Y - matrix(Y.mean, nrow = ntime, ncol = nloc, byrow = TRUE)
    ...
    loss <- function(q, u) {
        e <- (Y.demean - matrix(q, nrow = ntime, ncol = nloc, byrow = TRUE))
        losst <- numeric(ntime)
        for (t in 1:ntime) {
            d <- kern(Xt[t, ], x) %*% e[t, ]
            losst[[t]] <- inner.norm(d) + inner.prod(u, d)
        }
        return(log(mean(losst)))
    }
    
    best.index <- which.max(apply(Xt, 1, FUN = function(x1) { norm(kern(x1, x)) } ))
    init.q <- Y.demean[best.index, ]
    init.u <- rep(1,nloc)/inner.norm(rep(1, nloc)) * (2 * alpha - 1)
    op <- optim(init.q, fn = loss, u = init.u, method = method, ...)$par
    return(op + Y.mean)
}


estimate2.quantile <- function(Y, Xt, Z = NULL, quant = 0.5, x = colMedians(Xt)) {
    ...
    eps <- 1e-10
    q.hat <- solve(wmat + 1e-10*diag(nloc), est.mean + est.var %*% init.u / 2)
    return(q.hat)
}
