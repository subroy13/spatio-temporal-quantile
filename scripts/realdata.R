...
rawdf <- read_csv('./data/EPM Data/my_epm1.csv')
df <- rawdf %>% 
...
226 / sqrt(nrow(traindf))   # since sqrt(nrow(traindf)) is approximately p (cluster size), this gives approx # of clusters
...
write_csv(df2, './data/my_epm1_processed.csv')  # write back as a processed data
...
{
    clustNum <- 12   # PASS the Cluster Number Here
    ...
    for (i in 1:length(quants)) {
        ...
        for (j in 1:nrow(Xfull)) {
            output <- estimate.quantile(Ytrain, Xtrain, quant = quant, x = Xfull[j, ])
            EstMat[j, ] <- output$q
