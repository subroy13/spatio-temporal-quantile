...
estimate.quantile <- function(Y, 
  ...
  #' @param X data-matrix of the covariate process 
  ...
  # choose kernel with appropriate bandwidth based on p and available info about locations
  if (is.null(Z)) {
    kern <- function(x1, x2) {
  ...
  kern.array = lapply(c(1:ntime), FUN = function(j) kern(Xt[j,], x))
  ...
  idx = which(abs(c(1:ntime) - best.index) < nbn)
  kmats = kern.array[idx]
  Ymat = Y[idx,]
  mean.calc = lapply(c(1:length(idx)),FUN = function(x1) (kmats[[x1]] %*% Ymat[x1,]))
  var.calc = lapply(c(1:length(idx)),FUN = function(x1) tcrossprod(mean.calc[[x1]], Ymat[x1,]))
...
# function to estimate the variance function from a given data
...
calc.for.tests <- function(q0, Y, Xt, Z = NULL, x = colMedians(Xt), normalize = TRUE) {
  ...
  # calculating eta_t 
  eta <- t(sapply(1:length(idx),FUN = function(i) (kern.array[[idx[i]]] %*% (muhat[i, ] - q0))))
...
#' # function to estimate simultaneous confidence band for several quantiles
#' scb.quantile <- function(Y, 
#'                          Xt, 
#'                          Z = NULL, 
#'                          quant = 0.5, 
#'                          x = colMedians(Xt), 
